...
local function draw_gui()
  ...
  local vis, open = r.ImGui_Begin(ctx, "🎙 Vocal Volume Macro v1.5", true)
  if not vis then
    return open
  end
  ...
  zcr_changed, gui.zcr_thresh = r.ImGui_SliderDouble(
    ctx, "##zcr", gui.zcr_thresh, 0.05, 0.40, "%.3f")
  ...
  r.ImGui_SetNextItemWidth(ctx, 350)
  sc_changed, gui.sc_thresh = r.ImGui_SliderDouble(
    ctx, "##sc", gui.sc_thresh, 200, sc_max, "%.0f Hz")

  r.ImGui_Text(ctx, "Consonant onset sensitivity (Flux)")
  r.ImGui_SetNextItemWidth(ctx, 350)
  flux_changed, gui.flux_thresh = r.ImGui_SliderDouble(
    ctx, "##flux", gui.flux_thresh, 0.05, 1.0, "%.2f")
  ...
  r.ImGui_SetNextItemWidth(ctx, 350)
  changed, gui.block_ms = r.ImGui_SliderDouble(
    ctx, "##blk", gui.block_ms, 5, 30, "%.0f ms")
  if changed then
    gui.analyzed      = false
    gui.blocks        = nil
    gui.segments      = nil
  ...
  changed, gui.target_db = r.ImGui_SliderDouble(
    ctx, "##tgt", gui.target_db, -40, -6, "%.1f dB")
  ...
  r.ImGui_SetNextItemWidth(ctx, 350)
  changed, gui.gate_db = r.ImGui_SliderDouble(
    ctx, "##gate", gui.gate_db, -60, -10, "%.1f dB")
  if changed and gui.analyzed then
    gui.preview_gains = compute_gains(
      gui.segments, gui.blocks, gui.target_db, gui.macro_pct / 100,
      gui.per_block, gui.smooth_attack, gui.smooth_release, gui.gate_db)
  ...
  r.ImGui_SetNextItemWidth(ctx, 350)
  changed, gui.gap_ms = r.ImGui_SliderInt(
    ctx, "##gap", gui.gap_ms, 10, 200, "%d ms")
  if changed then
  ...
  r.ImGui_SetNextItemWidth(ctx, 350)
  changed, gui.macro_pct = r.ImGui_SliderInt(
    ctx, "##mac", gui.macro_pct, 0, 100, gui.macro_pct .. "%%")
  if changed and gui.analyzed then
    gui.preview_gains = compute_gains(
      gui.segments, gui.blocks, gui.target_db, gui.macro_pct / 100, gui.per_block, gui.smooth_attack, gui.smooth_release, gui.gate_db)
  end
  ...
  r.ImGui_SetNextItemWidth(ctx, 350)
  changed, gui.fade_ms = r.ImGui_SliderDouble(
    ctx, "##fad", gui.fade_ms, 1, 20, "%.0f ms")
  ...
  if gui.per_block then
    ...
    r.ImGui_SetNextItemWidth(ctx, 350)
    local att_changed, new_att = r.ImGui_SliderDouble(ctx, "##att", gui.smooth_attack, 0.01, 0.50, "Attack %.2f")
    if att_changed then
    ...
    r.ImGui_SetNextItemWidth(ctx, 350)
    local rel_changed, new_rel = r.ImGui_SliderDouble(ctx, "##rel", gui.smooth_release, 0.01, 0.50, "Release %.2f")
    if rel_changed then
